...
class Perceptron:
    ...
    def update(self, values, train, eta=.1):
        ...
        for i,e in zip(train,values):
            print "============="
            print "i:{:}, e:{:}, weights:{:}".format(i, e, self.weights)
            print "activate:{:}".format(self.activate(e))
            diff = np.subtract(i,self.activate(e))
            e = e.astype(np.float64)
            delta = np.multiply(eta*diff,e)
            print "diff:{:}, eta*diff:{:}, delta:{:}.".format(diff, eta*diff, delta)
            print "Antes: {:}".format(self.weights)
            self.weights = np.add(delta , self.weights.astype(np.float64))
            print "Depois: {:}".format(self.weights)
